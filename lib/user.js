...
User.prototype = {
  ...
  add_device: function(attributes){
    ...
    var device_hash = crypto.createHash('sha1').update(current_date + random).digest('hex').slice(0, 16);
    //store the details for the device
    attributes.id = device_hash;
    redis.hmset('device::' + device_hash, attributes);
    //store a pointer to the device in the user object
    self.device_ids.push(device_hash);
    self.save();
    //store a pointer to the device in the device list
    redis.sadd('devices', device_hash);
  }
}
